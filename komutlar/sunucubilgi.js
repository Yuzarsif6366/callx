...
exports.run = async (client, message, params) => {  
  ...
  .setAuthor(`Callx!-Bot Sunucu Bilgi`, message.guild.iconURL)
  .setColor("333333")
  .addField(`<a:ates:721696065203732481>Kullanıcılar [${message.guild.memberCount}]`, `${message.guild.members.filter(m => m.user.presence.status === "online").size} Aktif \n ${message.guild.members.filter(m => m.user.presence.status === "idle").size} Boşta \n  ${message.guild.members.filter(m => m.user.presence.status === "dnd").size} Rahatsız Etmeyin \n ${message.guild.members.filter(m => m.user.presence.status === "offline").size} Çevrim Dışı \n   ${message.guild.members.filter(m => m.user.bot).size} Bot`, true)
  .addField(`<a:ates:7721696065203732481> Kanallar`, `Toplam Kanal Sayısı : ${message.guild.channels.size} \nToplam Yazı Kanalı : ${message.guild.channels.filter(c => c.type === "text").size} \nToplam Ses Kanalı : ${message.guild.channels.filter(c => c.type === "voice").size}`, true)
  .addField(`<a:ates:721696065203732481> Sunucu Kurucusu`, message.guild.owner, true)
  .addField(`<a:ates:721696065203732481> Rol Sayısı`, `${message.guild.roles.size}`, true)
  .addField(`<a:ates:721696065203732481> Doğrulama Seviyesi`, message.guild.verificationLevel, true)
  
  .addField(`<a:ates:721696065203732481> Sunucu Bölgesi`, konum, true)
  .addField(`<a:ates:721696065203732481> Oluşturulma Tarihi`, tarih, true)
  .addField(`<a:ates:721696065203732481> Sunucu İD`,message.guild.id ,true)
  
   .addField('<a:ates:721696065203732481> Roller:', message.guild.roles.map(role => role.name).join(', '), true)
